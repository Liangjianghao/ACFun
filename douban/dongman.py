...
f=open('dm12.txt','w+',encoding='utf-8')
while 1:
    print('正在抓取第%s页'%page)
    dmUrl='https://movie.douban.com/j/new_search_subjects?sort=U&range=0,10&tags=%s&start=%s'%(type,page*20)
    print(dmUrl)
    ...
    for dm in dmArr:
        ...
        rate=dm['rate']
        print(title)
        f.write(title+'----'+rate+'----'+directors+'----'+id+'----'+url+'\n')
    page+=1
f.close()
