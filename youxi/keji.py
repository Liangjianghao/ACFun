...
#   PRIMARY KEY (`Id`)
# ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='a站科技表';
...
def getdace():
    sql = "SELECT *FROM actable where video_type LIKE '%科技%' ORDER BY video_uploadtime"
    cursor.execute(sql)
    connect.commit()
...
def analyVideo(index,video):
    ...
    sql="insert into %s(video_title,video_author,video_uploadtime,playNum,allPlayNum)values ('%s','%s','%s','%s', '%s')"%(tableName,video_title,video_author,video_uploadtime,playNum,allXjNum)
    cursor.execute(sql)
    connect.commit()
