...
baseTable='youxiTable'
tableName='youxiTableF'
baseTime='2010-10-1'

# CREATE TABLE `youxiTableF` (
...
def getinfor(timeStr):
    ...
    # sql = "SELECT * FROM %s  WHERE video_uploadtime<='%s' ORDER BY allPlayNum desc limit 20"%(baseTable,timeStr)
    # sql="SELECT * FROM %s WHERE video_uploadtime<'%s' GROUP BY video_author ORDER BY allPlayNum DESC LIMIT 20"%(baseTable,timeStr)
    # sql="SELECT  *,SUM(playNum) AS allP FROM youxitable WHERE video_uploadtime<'%s' GROUP BY video_author ORDER BY allp DESC"
