...
baseTable='actable'
tableName='youxiTable'
baseTime='2008-6-1'

# CREATE TABLE `youxiTable` (
...
#   PRIMARY KEY (`Id`)
# ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='a站游戏结果表';
...
def getdace():
    sql = "SELECT *FROM actable where video_type LIKE '%游戏%' ORDER BY video_uploadtime"
    cursor.execute(sql)
    connect.commit()
...
def analyVideo(index,video):
    ...
    sql='SELECT *,SUM(playNum) AS allPlayNum FROM (SELECT * FROM youxiTable ORDER BY allPlayNum DESC LIMIT 200000) t WHERE video_author="%s" '%video['video_author']
    cursor.execute(sql)
    connect.commit()
    ...
    sql="insert into youxiTable(video_title,video_author,video_uploadtime,playNum,allPlayNum)values ('%s','%s','%s','%s', '%s')"%(video_title,video_author,video_uploadtime,xjNum,allXjNum)
    cursor.execute(sql)
    connect.commit()
