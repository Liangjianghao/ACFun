...
baseTable='actable'
tableName='yinyueTable'
baseTime='2008-6-1'

# CREATE TABLE `yinyueTable` (
...
#   PRIMARY KEY (`Id`)
# ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='a站音乐结果表';
...
def getdace():
    sql = "SELECT *FROM actable where video_type LIKE '%音乐%' ORDER BY video_uploadtime"
    cursor.execute(sql)
    connect.commit()
