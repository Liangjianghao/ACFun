...
#   PRIMARY KEY (`Id`)
# ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='a站美食表'
def insertChatContent(video_title,video_url,video_author,video_uploadtime, playNum, danmuNum, comments, saveNum, xjNum):
    ...
    sql = "INSERT INTO ACmeishi (video_title,video_url,video_author,video_uploadtime,playNum,danmuNum,comments,saveNum,xjNum) VALUES ('%s','%s', '%s','%s','%s','%s', '%s','%s','%s')" % (video_title,video_url,video_author, video_uploadtime, playNum, danmuNum, comments, saveNum, xjNum)
    print(sql)
    insertSql = "INSERT INTO ACmeishi (video_title,video_url,video_author,video_uploadtime,playNum,danmuNum,comments,saveNum,xjNum)SELECT '%s','%s', '%s','%s','%s','%s', '%s','%s','%s' FROM ACmeishi WHERE NOT EXISTS(SELECT video_url FROM ACmeishi WHERE video_url='%s')LIMIT 1" % (video_title,video_url,video_author, video_uploadtime, playNum, danmuNum, comments, saveNum, xjNum,video_url)
